=encoding UTF-8

=head1 NAME

CLDR::Number::TODO - TODO document for CLDR::Number

=head1 TODO

=head2 Specification

=over

=item * currency spacing rules

=item * non-Latin (C<latn>) numbering systems (C<thai>, C<geor>, C<hant>, etc.)

=item * default numbering systems

=item * other numbering systems (C<native>, C<traditional>, C<finance>)

=item * format lengths (C<full>, C<long>, C<medium>, C<short>, C<narrow>)

=item * currency symbol lengths

=item * C<accounting> currency format in addition to default C<standard>

=item * scientific notation formatter

=item * superscripting exponent format for scientific notation

=back

=head2 Optimization

=over

=item * preparsed patterns for predefined locales

=item * locale subtag attributes for use without locale attribute parsing

=item * passing prebuilt locales to formatters from CLDR::Number

=item * parsed pattern caching

=back

=head1 SEE ALSO

=over

=item * L<Issues for CLDR::Number on GitHub|https://github.com/perl-cldr/cldr-number-perl5/issues>

=back
